...
elseif varsGUI.propDevice==2 % MTw Awinda
    input('Connect MTw Awinda, run its OS handler and then press enter to continue');
    t = xsensimus_handler.connect('localhost', 5000);
end
...
elseif varsGUI.propDevice==2 % MTw Awinda
    ...
    % Close MTw Awinda socket
    xsensimus_handler.disconnect(t);
end
