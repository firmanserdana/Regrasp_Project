function [stim_string,safe,cmdDelay] = make_stim_string_vMultiCmd(mode,stim_params,elec,safeLimit)

safe = 1;               % flag for stim SAFE = below safety limits
cmdDelay = 0;           % delay between multiple commands [s]
...
TD = 0;                 % [ms]
amp_step_size = 7.5;    % [uA]   TO CHECK THIIIIIIIIS for Summit
...
switch mode
    ...
    case 1          % ----- Single pulse -----
        ...
        stim_string{1} = [elec_string num2str(elec) ',;' TL_string num2str(TL) ',;' ...
            freq_string num2str(freq) ',;' PW_string num2str(PW) ',;' ...
            amp_string num2str(amp_steps) ',;' TD_string num2str(TD) ',;' ...
            FS_string num2str(FS) ',;' PL_string num2str(PL) ',;'];
    ...
    case 3          % ----- Recruitment curves -----
        ...
        n_incr = floor((amp_max-amp_min)/amp_incr) + 1;
        reps = stim_params.('RC_reps');
        PW = stim_params.('RC_PW')*1e-3;            % [ms]
        freq = stim_params.('RC_freq');             % [Hz]
        TL = reps/freq*1e3;                         % [ms] TL = (reps-0.5)/freq*1e3;
        cmdDelay = reps*1/freq;                     % [s]

        for iA = 1:n_incr
            amp = amp_min + (iA-1)*amp_incr;        % [uA]
            amp_steps = floor(amp / amp_step_size); % [steps]

            stim_string{iA} = [elec_string num2str(elec) ',;' TL_string num2str(TL) ',;' ...
                freq_string num2str(freq) ',;' PW_string num2str(PW) ',;' ...
                amp_string num2str(amp_steps) ',;' TD_string num2str(TD) ',;' ...
                FS_string num2str(FS) ',;' PL_string num2str(PL) ',;'];
    ...
    case 4          % ----- AM Bursts -----
        ...
        for iA = 1:n_incr
            ...
            for iR = 1:reps
                stim_string{(iA-1)*reps+iR} = [elec_string num2str(elec) ',;' TL_string num2str(TL) ',;' ...
                    freq_string num2str(freq) ',;' PW_string num2str(PW) ',;' ...
                    amp_string num2str(amp_steps) ',;' TD_string num2str(TD) ',;' ...
                    FS_string num2str(FS) ',;' PL_string num2str(PL) ',;'];
            end
    ...
    case 5          % ----- FM Bursts -----
        ...
        for iF = 1:n_incr
            ...
            for iR = 1:reps
                stim_string{(iF-1)*reps+iR} = [elec_string num2str(elec) ',;' TL_string num2str(TL) ',;' ...
                    freq_string num2str(freq) ',;' PW_string num2str(PW) ',;' ...
                    amp_string num2str(amp_steps) ',;' TD_string num2str(TD) ',;' ...
                    FS_string num2str(FS) ',;' PL_string num2str(PL) ',;'];
            end
